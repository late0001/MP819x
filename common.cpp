...
void myprintf(char *fmt,...) 
{
	va_list argp;
	va_start(argp, fmt);
	vsnprintf(msgBuf,1024, fmt, argp);
	va_end(argp);
#if 0
	int size=MultiByteToWideChar(CP_ACP, 0, msgBuf, -1, NULL, 0);
	wchar_t *ch=new wchar_t[size+1];
	if(!MultiByteToWideChar(CP_ACP, 0, msgBuf, -1, ch, size))
	{
		OutputDebugString(L"×Ö·û×ª»»Òì³£");
		delete []ch;
		return;
	}
...
void mysprintf(wchar_t *buf, char *fmt,...) 
{
	char msgBuf[1024]={0};
	va_list argp;
	va_start(argp, fmt);
	...
	if(!MultiByteToWideChar(CP_ACP, 0, msgBuf, -1, buf, size))
	{
		OutputDebugString("×Ö·û×ª»»Òì³£");
		return;
	}
...
int print_power_3(char *result)
{
	...
	// WT_RES_POWER_FRAME_DB
		...
		sprintf(buf, "Power frame: -- dBm\n");
		print_result += string(buf);

	// WT_RES_POWER_ALL_DB
		sprintf(buf, "Power all:   -- dBm\n");
		print_result += string(buf);

	// WT_RES_POWER_PEAK_DB
		sprintf(buf, "Power peak:  -- dBm\n");
		print_result += string(buf);
...
int print_evm_3( char *result)
{
	...
	string print_result;
		...
		sprintf(buf, "EVM all:     -- dB\n");
		print_result += string(buf);

		sprintf(buf, "EVM peak:    -- dB\n");
		print_result += string(buf);
	
		sprintf(buf, "EVM pilot:   -- dB\n");
		print_result += string(buf);
...
int print_freq_err_3( char *result)
{

	char buf[256] = {0};
	string print_result;
	
	sprintf(buf, "Freq err:    -- Hz\n");
	print_result += string(buf);
...
int print_datarate_2(int id, char *result)
{
	double data_result = 0;
	char description[128] = {0};
	char unit[128] = {0};
	...
	int ret = WT_GetResult(id, WT_RES_FRAME_DATA_RATE_MBPS, &data_result, description, unit);
	if(WT_ERR_CODE_OK == ret)
	{
		sprintf(buf, "Data Rate : %lf Mbps\n", data_result);
		print_result += string(buf);
	}
	ret = WT_GetResult(id, WT_RES_FRAME_PSDU_LENGTH, &data_result, description, unit);
	if(WT_ERR_CODE_OK == ret)
	{
		sprintf(buf, "PSDU: %lf  \n", data_result);
		print_result += string(buf);
...
int find_int_value(string buf, string token)
{
	char *ptr = NULL;
	char *p = NULL;
	vector<string>lines;
	...
	for(vector<string>::size_type i = 0; i < lines.size(); i++)
	{
		...
		if(regex_match(lines.at(i), std::regex("^\\s*\/\/{1}.*$")))//Æ¥Åä×¢ÊÍÐÐ£¬ÊÇ×¢ÊÍÐÐ£¬ÂÔ¹ýÈ¥
		{
				continue;
		}
		if(regex_match(lines.at(i), std::regex("^.*\/\/{1}.*$")))//ÊÇ·ñÓÐ×¢ÊÍ
		{
			lines.at(i) = regex_replace(lines.at(i), std::regex("\/\/.*$"), string(""));//ÂÔ¹ý×¢ÊÍ
		}
